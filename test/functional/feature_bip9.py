...
class Bip9Test(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 4
        self.setup_clean_chain = True
        self.extra_args = [
            ['-vbparams=testdummy:-2:-2'], # Node 0 has TestDummy NEVER_ACTIVE
            ['-vbparams=testdummy:{}:{}'.format(int(time.time()), int(time.time() + 100))], # Node 1 has regular activation window
            ['-vbparams=testdummy:{}:{}:{}'.format(int(time.time()), int(time.time() + 100), 144 * 5)], # Node 2 has minimum activation height
            ['-vbparams=testdummy:-2:-2'], # Node 3 has TestDummy NEVER_ACTIVE, but will be restarted with different params
        ]
    ...
    def test_height(self, height, status, mine_from, restart):
        ...
        for (i, node), st in zip(enumerate(self.nodes), status):
            ...
            if st is None:
                ...
                assert_equal(info["softforks"]["testdummy"]["bip9"]["status"], st)
        ...
        if restart:
            ...
            if restart.status is None:
                ...
                assert_equal(info["softforks"]["testdummy"]["bip9"]["status"], restart.status)
            ...
            if status[restart.node] is None:
                ...
                assert_equal(info["softforks"]["testdummy"]["bip9"]["status"], status[restart.node])

    def run_test(self):
        self.log.info("Checking -vbparams")
        self.stop_node(3)
        # TODO add BIP9 -vbparams consistency checks similar to https://github.com/bitcoin/bitcoin/pull/21392/commits/c95287e378a4a4a79b9632d7647015e96dc3a21c
        ...
        self.height = 0
        self.test_height(0, [None, "defined", "defined", None], None, RestartedNodeInfo(3, ["-vbparams=testdummy:{}:{}".format(int(time.time()), int(time.time() + 100))], "defined"))
        ...
        self.test_height(144-2, [None, "defined", "defined", None], 0, RestartedNodeInfo(3, ["-vbparams=testdummy:{}:{}".format(int(time.time()), int(time.time() + 100))], "defined"))
        self.test_height(144-1, [None, "started", "started", None], 0, RestartedNodeInfo(3, ["-vbparams=testdummy:{}:{}".format(int(time.time()), int(time.time() + 100))], "started"))
