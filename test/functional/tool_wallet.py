...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def test_encrypted_backup(self):
        ...
        assert backup_bytes[:6] == b"BIP138", f"Expected magic 'BIP138', got: {backup_bytes[:6]}"
        ...
        # Account recipients are padded with decoys to the smallest bucket.
        ...
        # Parse decrypted BIP380 descriptor backup content
        import json
        ...
        # Every account in the backup must be able to decrypt, including those
        # whose descriptor sorts after the first set.
        for xpub in {re.search(r'tpub[A-Za-z0-9]+', desc).group(0) for desc in original_desc_strs}:
            p = self.bitcoin_wallet_process(f"-pubkey={xpub}", "decryptbackup")
            account_output, stderr = p.communicate(input=backup_base64)
            assert_equal(p.poll(), 0)
            assert_equal(account_output.strip(), decrypted_content)
        ...
        p = self.bitcoin_wallet_process(f"-wallet={compact_import_wallet_name}", f"-pubkey={compact_xpub}", "decryptbackup")
