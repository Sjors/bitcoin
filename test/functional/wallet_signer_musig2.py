...
class WalletSignerMuSig2Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Unload the hot wallet so its name doesn't collide with the registered
        ...
        self.test_registered_musig2_descriptor()
    ...
    def test_display_address(self):
        ...
        # information from a registered descriptor.
    ...
    def _make_musig2_descriptor(self):
        ...
        importing into a registered-descriptor wallet with no private keys)."""
    ...
    def test_registered_musig2_descriptor(self):
        self.log.info("Test registered MuSig2 descriptor displayaddress via mock signer")
        # Build a 2-of-2 MuSig2 descriptor using two cosigner wallets on
        # node 0; the external-signer wallet on node 1 will import the xpub-only
        ...
        # only SPKM kind that registration dispatch in CWallet recognises.
        # The fingerprint just has to match the registration the wallet
        # stores after registerdescriptor; it does NOT need to be one of the
        ...
        # External-signer wallet on node 1 with no private keys.
        self.nodes[1].createwallet(
            wallet_name='hww_registered',
            disable_private_keys=True,
            external_signer=True,
        )
        hww = self.nodes[1].get_wallet_rpc('hww_registered')

        # Register the descriptor so CWallet::DisplayAddress can return the
        # opaque registration to the matching signer.
        reg = hww.registerdescriptor()
        assert_equal(len(reg["registrations"]), 1)
        assert_equal(reg["registrations"][0]["fingerprint"], device_fp)
        info = hww.getwalletinfo()
        assert_equal(len(info["external_signer_registrations"]), 1)
        assert_equal(info["external_signer_registrations"][0]["fingerprint"], device_fp)
        ...
        # CWallet::DisplayAddress should dispatch through the registered
        # descriptor path (rather than the single-key
