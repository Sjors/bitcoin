...
void Sv2TemplateProvider::RequestTransactionData(Sv2Client& client, node::Sv2RequestTransactionDataMsg msg)
{
...

    {
        ...
        fees = (*cached_block->second).getTxFees();
    }
    ...
    std::vector<std::pair<CTransactionRef, CAmount>> tx_fee_pairs;
    for (size_t i = 0; i < txs.size(); i++) {
        tx_fee_pairs.emplace_back(std::make_pair(txs[i], fees[i]));
    }

    node::Sv2RequestTransactionDataSuccessMsg request_tx_data_success{msg.m_template_id, std::move(witness_reserve_value), std::move(tx_fee_pairs)};
