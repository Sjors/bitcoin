...
void Sv2Connman::EventGotData(Id id, std::span<const uint8_t> _data)
{
    AssertLockNotHeld(m_clients_mutex);
    ...
    // Workaround for ReceivedBytes which currently wants a Span, not std::span.
    // TODO: replace Span with std::span in Sv2Noise and Sv2Transport, after
    //       https://github.com/bitcoin/bitcoin/pull/31519
    Span<const uint8_t> data = _data;
