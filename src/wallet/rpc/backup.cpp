...
static UniValue ProcessDescriptorImport(CWallet& wallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    try {
        ...
        if (multipath_normalized) {
            ...
            if (!wallet.AddMultipathDescriptor(batch, MultipathDescriptorRecord(std::move(*multipath_normalized), std::move(desc_ids)))) {
                warnings.push_back("Failed to write multipath descriptor record");
            }
        }
