...
RPCHelpMan derivehdkey()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            std::vector<CExtKey> wallet_xprvs;
            for (auto* spkm : spkms) {
                auto* desc_spkm{dynamic_cast<DescriptorScriptPubKeyMan*>(spkm)};
                CHECK_NONFATAL(desc_spkm);
                LOCK(desc_spkm->cs_desc_man);
                ...
                for (const CExtPubKey& xpub : desc_xpubs) {
                    ...
                    CHECK_NONFATAL(key);
                    wallet_xprvs.emplace_back(xpub, *key);
                }
            }

            if (wallet_xprvs.size() == 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "unused(KEY) descriptor not found");
            // TODO: add option to specify the xpub
            CHECK_NONFATAL(wallet_xprvs.size() == 1);

            std::optional<std::pair<CExtKey, KeyOriginInfo>> child{DeriveExtKey(wallet_xprvs[0], path)};
