...
void SQLiteDatabase::Open(int additional_flags)
{
    ...
    if (m_db == nullptr) {
        ...
        // The constructor Assert guarantees that the file path is absent if
        // and only if SQLITE_OPEN_MEMORY is set.
        const std::string file_path{m_file_path ? fs::PathToString(*m_file_path) : ":memory:"};
        int ret = sqlite3_open_v2(file_path.c_str(), &m_db, flags, nullptr);
        if (ret != SQLITE_OK) {
