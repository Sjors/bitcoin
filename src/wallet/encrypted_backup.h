...
/**
 * Create an encrypted backup from descriptors and a plaintext payload.
 *
 * @param[in] descriptors The wallet descriptors (used to derive the encryption key)
...
/** Create an encrypted backup using the recipients of a single descriptor. */
util::Result<EncryptedBackup> CreateEncryptedBackup(
