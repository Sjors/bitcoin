                          ...
                          std::vector<OutputGroup>& ret_discarded_groups)
{
    ...
    if (!coin_sel_params.m_avoid_partial_spends) {
        ...
        for (const auto& [type, outputs] : coins.coins) {
            for (const COutput& output : outputs) {
                // Get mempool info
                size_t clustersize, dummy;
                wallet.chain().getTransactionAncestry(output.outpoint.hash, dummy, clustersize);

                // Create a new group per output and add it to the all groups vector
                OutputGroup group(coin_sel_params);
                group.Insert(std::make_shared<COutput>(output), clustersize);
    ...
    const auto& insert_output = [&](
            const std::shared_ptr<COutput>& output, OutputType type, size_t clustersize,
        ...
        group->Insert(output, clustersize);
    };
    ...
    for (const auto& [type, outs] : coins.coins) {
        for (const COutput& output : outs) {
            ...
            if (output.GetEffectiveValue() > 0) {
                insert_output(shared_output, type, clustersize, spk_to_positive_groups_map);
            }

            // 'All' groups
            insert_output(shared_output, type, clustersize, spk_to_groups_map);
        }
    }
...
{
    unsigned int limit_cluster_count = 0;
    wallet.chain().getPackageLimits(limit_cluster_count);
    const size_t max_cluster_count = (size_t)std::max<int64_t>(1, limit_cluster_count);
    ...
    util::Result<SelectionResult> res = [&] {
        ...
        if (wallet.m_spend_zero_conf_change) {
            ...
            ordered_filters.push_back({CoinEligibilityFilter(0, 1, std::min(size_t{4}, max_cluster_count/3))});
            ordered_filters.push_back({CoinEligibilityFilter(0, 1, max_cluster_count/2)});
            // If partial groups are allowed, relax the requirement of spending OutputGroups (groups
            // of UTXOs sent to the same address, which are obviously controlled by a single wallet)
            // in their entirety.
            ordered_filters.push_back({CoinEligibilityFilter(0, 1, max_cluster_count-1, /*include_partial=*/true)});
            // Try with unsafe inputs if they are allowed. This may spend unconfirmed outputs
            // received from other wallets.
            if (coin_selection_params.m_include_unsafe_inputs) {
                ordered_filters.push_back({CoinEligibilityFilter(/*conf_mine=*/0, /*conf_theirs*/0, max_cluster_count-1, /*include_partial=*/true)});
            }
        ...
        for (const auto& group : discarded_groups) {
            ...
            if (group.m_cluster_count >= max_cluster_count) total_unconf_long_chain += group.GetSelectionAmount();
        }
