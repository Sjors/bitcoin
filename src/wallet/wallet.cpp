...
std::vector<std::string> CWallet::SetupDescriptorScriptPubKeyManPair(WalletBatch& batch, const CExtKey& master_key, OutputType output_type, bool receive, bool change)
{
    ...
    // descriptor. Its normalized form (which requires the master xprv) is
    // reconstructed from the expanded descriptors.
    ...
    Assert(descs.size() == 2);
    std::optional<std::string> multipath_normalized{ToNormalizedMultipathString(std::vector<const Descriptor*>{descs.at(0).get(), descs.at(1).get()}, keys)};
    Assert(multipath_normalized);
