...
std::optional<PSBTError> CWallet::FillPSBT(PartiallySignedTransaction& psbtx, bool& complete, std::optional<int> sighash_type, bool sign, bool bip32derivs, size_t * n_signed, bool finalize)
{
    ...
    auto fill_pass = [&](PartiallySignedTransaction& target) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet) -> std::optional<PSBTError> {
        ...
        for (ScriptPubKeyMan* spk_man : GetAllScriptPubKeyMans()) {
            ...
            error = spk_man->FillPSBT(target, txdata, sighash_type, sign, bip32derivs, &n_signed_this_spkm, finalize);
            if (error) {
                return error;
            }
    ...
    if (sign && !complete && musig2_round2_ready(psbtx)) {
        ...
        // fill_pass writes through to outer `complete`; preserve the
        // round-1 value when the round-2 attempt didn't pan out so
        // callers see the stable round-1 result.
        if (!probe_err && complete) {
...
util::Result<void> CWallet::DisplayAddress(const CTxDestination& dest)
{
    ...
    for (const auto& spk_man : GetScriptPubKeyMans(scriptPubKey)) {
        ...
        // descriptor against the previously-stored hmac) rather than falling
        // back on the single-key descriptor path that InferDescriptor produces.
        if (IsCandidateForBIP388Policy(*signer_spk_man)) {
            // TODO: support multiple registered policies on a single wallet by
            // matching (template, keys) against the stored hmac. For now we
            // pick the first hmac registered for this signer's fingerprint.
            const BIP388* matching_hmac{nullptr};
            for (const auto& entry : m_bip388) {
                if (entry.fingerprint == signer->m_fingerprint) {
                    matching_hmac = &entry;
