...
static DBErrors LoadWalletPolicyRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    LoadResult res = LoadRecords(pwallet, batch, DBKeys::BIP388_HMAC,
        ...
        pwallet->LoadHmacBIP388(policy_name, fingerprint, hmac.empty() ? std::nullopt : std::optional<std::string>{hmac});
        return DBErrors::LOAD_OK;
    });
...
bool WalletBatch::WriteHmacBip388(const std::string& policy_name, const std::string& fingerprint, const std::optional<std::string>& hmac)
{
    return WriteIC(std::make_pair(DBKeys::BIP388_HMAC, std::make_pair(policy_name, fingerprint)), hmac.value_or(""));
}
