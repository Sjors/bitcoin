...
public:
    ...
    std::unique_ptr<BlockTemplate> createNewBlock(const BlockCreateOptions& options, bool cooldown) override
    {
        ...
        auto new_template = block_template_manager().CreateNewTemplate(create_options, &template_id);
        return std::make_unique<BlockTemplateImpl>(create_options, std::move(new_template), template_id, m_node);
    }
