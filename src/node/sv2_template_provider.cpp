    ...
    case Sv2MsgType::REQUEST_TRANSACTION_DATA:
    {
        ...
        if (cached_block != m_block_template_cache.end()) {
            ...
            if (block.hashPrevBlock != m_best_prev_hash) {
                ...
                LogPrintLevel(BCLog::SV2, BCLog::Level::Debug, "Send 0x75 RequestTransactionData.Error (stale-template-id) to client id=%zu\n",
                          client.m_id);
            ...
            LogPrintLevel(BCLog::SV2, BCLog::Level::Debug, "Send 0x75 RequestTransactionData.Error (template-id-not-found) to client id=%zu\n",
                          client.m_id);
            client.m_send_messages.emplace_back(request_tx_data_error);
        }
