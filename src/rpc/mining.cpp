...
static UniValue generateBlocks(ChainstateManager& chainman, node::BlockTemplateManager& block_template_manager, const CScript& coinbase_output_script, int nGenerate, uint64_t nMaxTries)
{
    ...
    while (nGenerate > 0 && !chainman.m_interrupt) {
        std::unique_ptr<node::CBlockTemplate> block_template{block_template_manager.CreateNewTemplate(node::MergeMiningOptions({.coinbase_output_script = coinbase_output_script}, block_template_manager.GetInitBlockCreateOptions()))};
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ChainstateManager& chainman = EnsureChainman(node);
    {
        ...
        {
            std::unique_ptr<node::CBlockTemplate> block_template{block_template_manager.CreateNewTemplate(node::MergeMiningOptions({.use_mempool = false, .coinbase_output_script = coinbase_output_script}, block_template_manager.GetInitBlockCreateOptions()))};
            CHECK_NONFATAL(block_template);
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - time_start > 5))
    {
        ...
        block_template = block_template_manager.CreateNewTemplate(node::MergeMiningOptions({}, block_template_manager.GetInitBlockCreateOptions()));
        CHECK_NONFATAL(block_template);
