        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[1].isNull()) {
        ...
        {
            LOCK(cs_main);
            pindex = g_chainman.m_blockman.LookupBlockIndex(coins_view->GetBestBlock());
        }
    }

    NodeContext& node = EnsureNodeContext(request.context);
    if (GetUTXOStats(coins_view, stats, node.rpc_interruption_point, pindex)) {
        ...
        if (!stats.from_index) {
            ...
            if (pindex->nHeight > 0) {
                GetUTXOStats(coins_view, prev_stats, node.rpc_interruption_point, pindex->pprev);
            }
...
UniValue CreateUTXOSnapshot(NodeContext& node, CChainState& chainstate, CAutoFile& afile)
{
...

    {
        ...
        if (!GetUTXOStats(&chainstate.CoinsDB(), stats, node.rpc_interruption_point)) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to read UTXO set");
        }
