...
static const CBlockIndex* GetTxProofBlockIndexByWtxid(ChainstateManager& chainman, const std::optional<Wtxid>& wtxid, const UniValue& blockhash_param)
{
    ...
    const auto result{g_wtxindex->FindTx(*wtxid)};
    if (!result || result->block_hash.IsNull()) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Transaction not yet in block");
    }

    LOCK(cs_main);
    const CBlockIndex* pblockindex{chainman.m_blockman.LookupBlockIndex(result->block_hash)};
    if (!pblockindex) {
