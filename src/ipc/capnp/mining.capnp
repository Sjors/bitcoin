...
interface Mining $Proxy.wrap("interfaces::Mining") {
    ...
    interrupt @6 () -> ();
    collectTxs @7 (context :Proxy.Context, wtxids: List(Data)) -> (result: TxCollection);
}
...
interface TxCollection $Proxy.wrap("interfaces::TxCollection") {
    ...
    unknownTxPos @2 (context: Proxy.Context) -> (result: List(UInt32));
    addMissingTxs @3 (context: Proxy.Context, txs: List(Data)) -> ();
}
