...
BaseIndex::DB::DB(const fs::path& path, size_t n_cache_size, bool f_memory, bool f_wipe, bool f_obfuscate, bool use_bloom_filter) :
    CDBWrapper{DBParams{
        .path = path,
        ...
        .options = [use_bloom_filter] {
            DBOptions options;
            node::ReadDatabaseArgs(gArgs, options);
            options.use_bloom_filter = use_bloom_filter;
            return options;
        }()}}
{}
