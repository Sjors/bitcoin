...
{
    Sv2Transport m_transport; //!< Sv2Transport being tested
    Sv2Cipher m_peer_cipher;  //!< Cipher to help with the other side
...
public:
    ...
    explicit Sv2TransportTester(bool test_initiator)
        : m_transport{test_initiator, GenerateRandomTestKey()},
          m_peer_cipher{GenerateRandomTestKey(), !test_initiator},
    ...
    InteractResult Interact()
    {
        ...
        while (true) {
            ...
            if (should_progress && (!progress || pretend_no_progress)) {
                Sv2NetMsg msg = m_transport.GetReceivedMessage();
    ...
    void ProcessHandshake1() {
        if (m_test_initiator) {
            ...
            m_peer_cipher.m_handshake_state->ReadMsgEphemeralPK(MakeWritableByteSpan(m_received));
            m_received.clear();
        } else {
            BOOST_REQUIRE(m_to_send.empty());
            m_to_send.resize(KEY_SIZE);
            m_peer_cipher.m_handshake_state->WriteMsgEphemeralPK(MakeWritableByteSpan(m_to_send));
        }
    ...
    void ProcessHandshake2()
    {
        if (m_test_initiator) {
            ...
            m_to_send.resize(INITIATOR_EXPECTED_HANDSHAKE_MESSAGE_LENGTH);
            m_peer_cipher.m_handshake_state->WriteMsgES(MakeWritableByteSpan(m_to_send));

            // At this point the peer is done with the handshake:
            m_peer_cipher.FinishHandshake();
        } else {
            BOOST_REQUIRE(m_received.size() == INITIATOR_EXPECTED_HANDSHAKE_MESSAGE_LENGTH);
            BOOST_REQUIRE(m_peer_cipher.m_handshake_state->ReadMsgES(MakeWritableByteSpan(m_received)));
            m_received.clear();
    ...
    void SendPacket(Sv2NetMsg msg)
    {
        ...
        const size_t encrypted_payload_size = Sv2NoiseSession::EncryptedMessageSize(msg.m_msg.size());
    ...
    void ReceiveMessage(Sv2NetMsg expected_msg)
    {
        ...
        Span<std::byte> received_span = MakeWritableByteSpan(m_received);
        Span<std::byte> encrypted_header = received_span.subspan(0, SV2_HEADER_ENCRYPTED_SIZE);
        BOOST_REQUIRE(m_peer_cipher.DecryptMessage(encrypted_header));


        Span<std::byte> header_span = encrypted_header.subspan(0, SV2_HEADER_PLAIN_SIZE);

        // Decode header
        DataStream ss_header{header_span};
        node::Sv2NetHeader header;
        ss_header >> header;

        BOOST_CHECK(header.m_msg_type == expected_msg.m_sv2_header.m_msg_type);
        ...
        BOOST_REQUIRE(m_peer_cipher.DecryptMessage(encrypted_payload));
        ...
        Sv2NetMsg message{std::move(header), std::move(decode_buffer)};
    ...
    {
        BOOST_CHECK(m_transport.NoiseHash() == m_peer_cipher.m_hash);
    }
...
BOOST_AUTO_TEST_CASE(sv2_transport_responder_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Handshake complete, have the initator send us a message:
        uint64_t template_id = InsecureRandRange(10000000);
        Sv2SetNewPrevHashMsg set_new_prev_hash{CBlock(), template_id};
        Sv2NetMsg msg{set_new_prev_hash};

        tester.SendPacket(msg);
        ret = tester.Interact();
        BOOST_REQUIRE(ret && ret->size() == 1);
        BOOST_CHECK((*ret)[0] &&
                    (*ret)[0]->m_sv2_header.m_msg_type == Sv2MsgType::SET_NEW_PREV_HASH);
