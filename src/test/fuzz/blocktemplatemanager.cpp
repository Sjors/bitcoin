...
FUZZ_TARGET(block_template_manager, .init = initialize_block_template_manager)
{
    ...
    int max_mempool_weight = template_creation_provider.ConsumeIntegralInRange<int>(0, DEFAULT_BLOCK_MAX_WEIGHT * 10);
    PopulateRandTransactionsToMempool(template_creation_provider, mempool, max_mempool_weight);
    ...
    if (template_creation_provider.ConsumeBool()) {
        const CAmount fee_paid = template_creation_provider.ConsumeIntegralInRange<CAmount>(0, COIN);
        const int32_t tx_size = template_creation_provider.ConsumeIntegralInRange<int32_t>(1, MAX_STANDARD_TX_WEIGHT / WITNESS_SCALE_FACTOR);
