...
BOOST_AUTO_TEST_CASE(handshake_test)
{
    ...
    std::vector<std::byte> payload_encrypted(plaintext.size() + POLY1305_TAGLEN, std::byte(0));
    auto payload_encrypted_span = MakeWritableByteSpan(payload_encrypted);
    noise_initiator.EncryptMessage(transport_header.m_payload, payload_encrypted_span);
    BOOST_CHECK_EQUAL(transport_header.m_payload.size(), plaintext.size());
