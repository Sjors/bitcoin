             ...
             std::optional<std::string> expected_prv = std::nullopt, std::optional<std::string> expected_pub = std::nullopt, int desc_index = 0)
{
    ...
    if (expect_multipath) {
        // The multipath string is normalized: hardened markers in key origins
        // always use 'h', regardless of how the input spells them.
        const std::string norm_pub{UseHInsteadOfApostrophe(pub)};
        const std::string expected{norm_pub + "#" + GetDescriptorChecksum(norm_pub)};
...
BOOST_AUTO_TEST_CASE(descriptor_test)
{
    ...
    // Malformed multipath descriptors with trailing input must fail before apostrophe canonicalization
    CheckUnparsable("tr(xprv9uPDJpEQgRQfDcW7BkF7eTya6RPxXeJCqCJGHuCJ4GiRVLzkTXBAJMu2qaMWPrS7AANYqdq6vcBcBUdJCVVFceUvJFjaPdGZ2y9WACViL4L/<44;114;7'>))",
                    "tr(xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/<44;114;7'>))", "");
    ...
    // A hex key typed in uppercase is lowercased in the multipath descriptor
    // string and the expanded descriptors
