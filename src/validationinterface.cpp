...
#define ENQUEUE_AND_LOG_EVENT(category, level, event, fmt, name, ...)                                 \
    do {                                                                                              \
        ...
        LogPrintLevel(category, BCLog::Level::Trace, "Enqueuing " fmt "\n", local_name, __VA_ARGS__); \
...
void CMainSignals::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "%s: new block hash=%s fork block hash=%s (in IBD=%s)", __func__,
...
void CMainSignals::TransactionAddedToMempool(const NewMempoolTransactionInfo& tx, uint64_t mempool_sequence)
{
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "%s: txid=%s wtxid=%s", __func__,
...
void CMainSignals::TransactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason, uint64_t mempool_sequence) {
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "%s: txid=%s wtxid=%s reason=%s", __func__,
                          tx->GetHash().ToString(),
...
void CMainSignals::BlockConnected(ChainstateRole role, const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindex) {
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "%s: block hash=%s block height=%d", __func__,
...
void CMainSignals::MempoolTransactionsRemovedForBlock(const std::vector<RemovedMempoolTransactionInfo>& txs_removed_for_block, unsigned int nBlockHeight)
{
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "%s: block height=%s txs removed=%s", __func__,
                          nBlockHeight,
...
void CMainSignals::ChainStateFlushed(ChainstateRole role, const CBlockLocator &locator) {
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "%s: block hash=%s", __func__,
                          locator.IsNull() ? "null" : locator.vHave.front().ToString());
}

void CMainSignals::BlockChecked(const CBlock& block, const BlockValidationState& state) {
    LogPrintLevel(BCLog::VALIDATION, BCLog::Level::Debug, "%s: block hash=%s state=%s\n", __func__,
              block.GetHash().ToString(), state.ToString());
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.BlockChecked(block, state); });
}

void CMainSignals::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock> &block) {
    LogPrintLevel(BCLog::VALIDATION, BCLog::Level::Debug, "%s: block hash=%s\n", __func__, block->GetHash().ToString());
