...
void CMainSignals::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "new block hash=%s fork block hash=%s (in IBD=%s)",
...
void CMainSignals::BlockConnected(ChainstateRole role, const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindex) {
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "block connected hash=%s block height=%d",
...
void CMainSignals::BlockDisconnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    ENQUEUE_AND_LOG_EVENT(BCLog::VALIDATION, BCLog::Level::Debug, event, "block disconnected hash=%s block height=%d",
                          pblock->GetHash().ToString(),
                          pindex->nHeight);
}
...
void CMainSignals::BlockChecked(const CBlock& block, const BlockValidationState& state) {
    LogPrintLevel(BCLog::VALIDATION, BCLog::Level::Debug, "block checked hash=%s state=%s\n",
              block.GetHash().ToString(), state.ToString());
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.BlockChecked(block, state); });
}

void CMainSignals::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock> &block) {
    LogPrintLevel(BCLog::VALIDATION, BCLog::Level::Debug, "new proof-of-work valid block hash=%s\n", block->GetHash().ToString());
