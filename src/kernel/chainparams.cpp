...
void CChainParams::ApplyDeploymentOptions(const DeploymentOptions& opts)
{
    for (const auto& [dep, height] : opts.activation_heights) {
        ...
        case Consensus::BuriedDeployment::DEPLOYMENT_CSV:
            ...
            break;
        case Consensus::BuriedDeployment::DEPLOYMENT_TAPROOT_V2:
            consensus.TaprootV2Height = int{height};
            break;
        }
    }
...
public:
    CMainParams(const MainNetOptions& opts) {
        ...
        consensus.TaprootV2Height = std::numeric_limits<int>::max(); // Not deployed
