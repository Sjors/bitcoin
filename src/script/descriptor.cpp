...
static std::optional<KeyPath> MultipathExtKeyNormalizationPrefix(const std::vector<KeyPath>& paths, DeriveType type)
{
    ...
    if (paths.size() > 1) {
        const auto [first_mismatch, _]{std::ranges::mismatch(paths[0], paths[1])};
        const size_t mismatch_pos(first_mismatch - paths[0].begin());
        if (std::ranges::any_of(paths, [mismatch_pos](const KeyPath& path) {
                const auto last_hardened{LastHardenedIndex(path)};
                return last_hardened && mismatch_pos <= *last_hardened;
            })) {
            return std::nullopt;
        }
    }
