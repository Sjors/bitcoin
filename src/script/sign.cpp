...
static bool SignTaproot(const SigningProvider& provider, const BaseSignatureCreator& creator, const WitnessV1Taproot& output, SignatureData& sigdata, std::vector<valtype>& result)
{
    ...
    for (const auto& item : sigdata.tr_spenddata.scripts) {
        std::vector<std::vector<unsigned char>> result_stack;
        if (SignTaprootScript(provider, creator, sigdata, item.first.second, item.first.first, result_stack)) {
            result_stack.emplace_back(std::begin(item.first.first), std::end(item.first.first)); // Push the script
            result_stack.push_back(item.second); // Push the control block
                     ...
                     std::vector<valtype>& ret, TxoutType& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    case TxoutType::WITNESS_V1_TAPROOT:
        return SignTaproot(provider, creator, WitnessV1Taproot(XOnlyPubKey{vSolutions[1]}), sigdata, ret);
